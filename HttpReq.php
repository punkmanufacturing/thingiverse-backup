...
class HttpReq extends CurlReq {
	...
	public $options = array(
			...
			CURLOPT_ENCODING => '',
			CURLOPT_USERAGENT => 'Mozilla/5.0 (Windows NT 6.1; rv:7.0.1) Gecko/20100101 Firefox/7.0.1',
	...
	protected function httperror($code) {
		...
		$this->keep = false;
		if($this->singleFail) {
			$this->event('fail', array('httperror '.$code));
		}
	}
	...
	static public function encodeArgs($args, $sep='&', $suffix=null, $prefix=null) {
		$str = http_build_query($args, $prefix, $sep);
		if(!is_null($suffix)) {
